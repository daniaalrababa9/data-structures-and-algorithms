...
const totalCharacters = (arr) => {
    ...
    let characters = 0;
    arr.forEach(value => {
        characters += 1 + value.children.length;
        if (value.spouse) {
            return characters += 1;
...
const houseSize = (arr) => {
    ...
    arr.forEach(value => {
        ...
        let members = 1 + value.children.length;
        if (value.spouse) {
            members += 1;
        } else {
            members += 0;
        }
        return sizes.push({ house, members });
    }
    )
    return sizes;
};
...
const houseSurvivors = (arr) => {
    ...
    arr.forEach(value => {
        const house = value.house;
        let members = 1 + value.children.length;
        if (value.spouse && !deceasedSpouses.includes(value.spouse)) {
            members += 1;
        } else {
            members += 0;
        }
